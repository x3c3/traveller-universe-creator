def export_sector(db, ss):
    ...
    from reportlab.platypus import (SimpleDocTemplate, Table, TableStyle, Paragraph,
                                    PageBreak, KeepTogether, Spacer, Image)
    ...
    # Create a type alias for the sqlite3 connection
    sqlite3_connection = sqlite3.Connection

    #   CLASSES

    class ProgramDetails:
        def __init__(self, database_path, subsector_letter):
            self.db_name = self._get_db(database_path)
            self.sector = self._get_sector()
            self.subsector = self._get_subsector(subsector_letter)
            self.output_file_name = self._get_output_file_name()
            self.image_path = self._get_image_path()
            self.elems = []

        @staticmethod
        def _get_db(database_path: str) -> str:
            logging.debug(f'db loading: {database_path}')
        ...
        @staticmethod
        def _get_subsector(subsector_letter: str) -> str:
            logging.debug(f'subsector chosen: {subsector_letter}')
            return subsector_letter

        def _get_output_file_name(self) -> str:
            return 'sector_db/subsector_' + self.subsector + '.pdf'
    ...
    class TextBeforeTable:
        ...
        def populate_system_text_before_table(self, system_details, ex_styles):
            self.header_line = Paragraph(f"(<b>{system_details.location}) {system_details.name}</b>",
                                         ex_styles.header_style)
            self.t5_line = Paragraph(f"{system_details.remarks}   {system_details.ix}   {system_details.ex}  \
                                     {system_details.cx}", ex_styles.wants_needs_style)

            self.detail_line = Paragraph(f"<b>Bases:</b> {system_details.bases}  \
                                         <b>Travel Zone</b>: {system_details.zone}   \
                                         <b>PBG:</b> {system_details.pbg}   \
                                         <b>Stellar:</b> {system_details.stars}", ex_styles.wants_needs_style)
        ...
        def populate_trader_text_before_table(self, trader_details, ex_styles):
            self.gurps_line = Paragraph(f"<b>WTN:</b> {trader_details.wtn}  \
                                           ...
                                           <b>Exchange:</b> {trader_details.exchange}", ex_styles.wants_needs_style)

            self.wants_line = Paragraph(f"<b>Surplus:</b> {trader_details.wants}", ex_styles.wants_needs_style)

            self.needs_line = Paragraph(f"<b>Needs:</b> {trader_details.needs} ", ex_styles.wants_needs_style)
    ...
    class CultureLines:
        ...
        def populate_culture_lines(self, culture_details, ex_styles):
            ...
            self.age_line = Paragraph(f"{culture_details.age.capitalize()}.  \
                                       ...
                                       {culture_details.tendency.capitalize()} tendency.", ex_styles.wants_needs_style)
            ...
            self.status_quo_line = Paragraph(f"<b>Spr: </b>{culture_details.spiritual_outlook.capitalize()}. \
                                              ...
                                              <b>Int: </b>{culture_details.interest.capitalize()}.",
                                             ex_styles.wants_needs_style)
            ...
            self.symbol_line = Paragraph(f"Mat{culture_details.materialism_symbol}  \
                                          ...
                                          Con{culture_details.consumerism_symbol}", ex_styles.wants_needs_style)

            self.skills_line = Paragraph(f"<b>Common skills:</b> {culture_details.common_skills}",
                                         ex_styles.wants_needs_style)
    ...
    class ExportPageStyles:
        ...
        def populate_styles(self):
            ...
            self.wants_needs_style = ParagraphStyle(
    ...
    def get_detail_stats_object(c, loc_orb, uwp_dict):
        ...
        detail_stats_object = DetailStats(orbital_detail_row[0][0], uwp_dict[loc_orb][0], orbital_detail_row[0][1],
                                          uwp_dict[loc_orb][1], orbital_detail_row[0][2], orbital_detail_row[0][3],
                                          orbital_detail_row[0][4], orbital_detail_row[0][5], orbital_detail_row[0][6],
                                          orbital_detail_row[0][7], round(orbital_detail_row[0][8], 2),
                                          orbital_detail_row[0][9], orbital_detail_row[0][10],
                                          orbital_detail_row[0][11],
                                          orbital_detail_row[0][12] + orbital_detail_row[0][14],
                                          orbital_detail_row[0][13], orbital_detail_row[0][15],
                                          orbital_detail_row[0][16], orbital_detail_row[0][17],
                                          orbital_detail_row[0][18], orbital_detail_row[0][19],
                                          orbital_detail_row[0][20], orbital_detail_row[0][21],
                                          orbital_detail_row[0][22], orbital_detail_row[0][23],
                                          journey_detail_row[0][0], journey_detail_row[0][1],
                                          int(round(journey_detail_row[0][2], 0)),
                                          int(round(journey_detail_row[0][3], 0)),
                                          int(round(journey_detail_row[0][4], 0)),
                                          int(round(journey_detail_row[0][5], 0)),
                                          int(round(journey_detail_row[0][6], 0)),
                                          int(round(journey_detail_row[0][7], 0)))
    ...
    def get_far_trader_object(c, location):
        ...
        trader_object = FarTraderDetails(
            trader_row[0][0],  # wtn
            trader_row[0][1],  # gwp
            round(trader_row[0][2], 2),  # exchange
            trader_row[0][3],  # needs
            trader_row[0][4])  # wants needs
    ...
    def fix_uwp(df):
        ...
        df = df.drop(
            ['starport', 'size', 'hydrographics', 'atmosphere', 'population', 'government', 'law', 'tech_level'],
            axis=1)
    ...
    def get_orbital_data(db_connection: sqlite3_connection, location: str) -> pd.DataFrame:
        ...
        df = pd.read_sql_query(table_sql, db_connection, params=(location,))  # Will produce an Unexpected Type Warning
    ...
    def get_summary_table_data(df):
        data = [
            ['Loc-Orb', 'Name', 'UWP', 'Gr', 'Ri', 'Yr', 'Dy', 'Atm', 'Comp', 'Temp', '1G', '2G', '3G', '4G', '5G',
             '6G'],
        ]
    ...
    def append_system_data(location,
        ...
        text_before_table = TextBeforeTable()
        ...
        uwp_dict = {loc_orbit_list: [name_list, uwp_list] for loc_orbit_list, name_list, uwp_list in
                    zip(loc_orbit_list, name_list, uwp_list)}
    ...
    def program_control(db, ss):
        program_details = ProgramDetails(db, ss)

        # Connect to DB

        conn: sqlite3.Connection = sqlite3.connect(program_details.db_name)
        cursor = conn.cursor()
        ...
        export_page_styles = ExportPageStyles()
